...
router.get("/fetchSubjects", authenticateUser, async (req, res) => {
  ...
  try
  {
    // user._id from authenticateUser middleware 
  ...
  {
    console.error("Error fetching subjects:",ror);
...
router.get("/fetchSets", authenticateUser,async (req, res) => {
  ...
  try
  {
    ...
    if(!setData)  return res.status(404).json({ error: "No sets found for this subject" });
